...
void childProcess()
{
    ...
    for(int i = 0; i < 30; ++i)
    {
        printf("child process sleeping zzz\n");
        sleep(1);
    }
...
void parentProcess()
{
    ...
    if(sem_trywait(semaphore) != 0)
    {
        ...
        if(status == 1)
        {
            printf("Kill child process with ID: %d\n", otherPid);
            kill(otherPid, SIGUSR1);
            printf("Killed child process\n");

            printf("Proving child process is killed, should see no dots response from SIGUSR2 signal\n");
            sleep(5);
            kill(otherPid, SIGUSR2);
            sleep(1);
